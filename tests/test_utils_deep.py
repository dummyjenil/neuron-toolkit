...
from onnx_toolkit._utils import _attr_value, _node_attrs, _build_shape_info, _ONNX_DTYPE_TO_NP
...
def test_attr_value_all_types():
    ...
    # STRING
    attr_s = helper.make_attribute("s", "hello".encode("utf-8"))
    assert _attr_value(attr_s) == "hello"

    # TENSOR
    tensor_val = np.array([1, 2, 3], dtype=np.float32)
    attr_t = helper.make_attribute("t", helper.make_tensor("t", TensorProto.FLOAT, [3], tensor_val.tobytes(), raw=True))
    ...
    # STRINGS
    attr_ss = helper.make_attribute("ss", ["a".encode("utf-8"), "b".encode("utf-8")])
