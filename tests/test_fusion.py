from neuron_toolkit import NeuronGraph, NeuronQuery, Pattern


def test_onnx_graph_entry_point(complex_model):
    # Test NeuronGraph initialization and basic methods
    graph = NeuronGraph(complex_model)

    # Test query()
    q = graph.query()
    assert isinstance(q, NeuronQuery)
    assert q.count() == 6
...
def test_graph_passes(simple_model):
    ...
    assert isinstance(graph.passes.fuse_conv_bn(), NeuronRewriter)
